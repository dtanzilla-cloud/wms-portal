...
export default function NewConsigneePage() {
  ...
  function updateAddress(i: number, field: keyof AddressRow, value: string | boolean) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const customerId = profile?.customer_id ?? (
        profile?.role === 'warehouse_staff' || profile?.role === 'admin' ? null : null
      )

      const { data: consignee, error: cErr } = await supabase
        .from('consignees')
        .insert({
              ...
              <div key={i} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <input
                      type="text" value={addr.label} onChange={e => updateAddress(i, 'label', e.target.value)}
                    ...
                    <label className="flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer">
                      <input
                        type="radio" name="default_addr" checked={addr.is_default}
                  ...
                  {addresses.length > 1 && (
                    <button type="button" onClick={() => removeAddress(i)} className="text-gray-400 hover:text-red-500">
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    ...
                    <input
                      type="text" required value={addr.address_line1} onChange={e => updateAddress(i, 'address_line1', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <input
                      type="text" value={addr.address_line2} onChange={e => updateAddress(i, 'address_line2', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text" required value={addr.city} onChange={e => updateAddress(i, 'city', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text" required value={addr.state} onChange={e => updateAddress(i, 'state', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="CA"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text" required value={addr.postal_code} onChange={e => updateAddress(i, 'postal_code', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text" value={addr.country} onChange={e => updateAddress(i, 'country', e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
