...
export async function POST(req: NextRequest) {
  try {
    ...
    const SITE_NAME = process.env.NEXT_PUBLIC_SITE_NAME ?? 'CTS Portal'
    const inviteUrl = `${APP_URL}/auth/accept-invite?token=${token}`
    const customerName = (consignee.customers as any)?.name ?? SITE_NAME

    const resend = new Resend(process.env.RESEND_API_KEY)
    const fromEmail = process.env.RESEND_FROM_EMAIL ?? `${SITE_NAME} <noreply@yourdomain.com>`

    await resend.emails.send({
      from: fromEmail,
      to: consignee.contact_email,
      subject: `You've been invited to ${SITE_NAME} by ${customerName}`,
...
<div style="max-width:560px;margin:40px auto;background:#fff;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
  <div style="background:#1d4ed8;padding:20px 28px">
    <span style="color:#fff;font-size:16px;font-weight:600">${SITE_NAME}</span>
  </div>
  <div style="padding:28px">
    ...
    <p style="color:#475569;font-size:14px;line-height:1.6;margin:0 0 16px">
      ...
      to access ${SITE_NAME}. You can view your inventory and track your incoming shipments.
    </p>
