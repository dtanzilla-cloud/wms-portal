...
export async function POST(req: NextRequest) {
  try {
    ...
    const customerName = (consignee.customers as any)?.name ?? 'In N Out'

    const resend = new Resend(process.env.RESEND_API_KEY)
    const fromEmail = process.env.RESEND_FROM_EMAIL ?? `In N Out <noreply@yourdomain.com>`

    await resend.emails.send({
      from: fromEmail,
      to: consignee.contact_email,
      subject: `You've been invited to In N Out by ${customerName}`,
...
<div style="max-width:560px;margin:40px auto;background:#fff;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden">
  ...
  <div style="padding:28px">
    <h2 style="margin:0 0 12px;font-size:18px;color:#1e293b">
      You've been invited to In N Out
    </h2>
    ...
    <p style="color:#475569;font-size:14px;line-height:1.6;margin:0 0 16px">
      ...
      to access In N Out. You can view your inventory and track your incoming shipments.
    </p>
